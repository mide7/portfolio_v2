...
import DotCircle from "@/public/images/dotcirclelabs.png";
import Ribbon from "@/public/images/ribbonprotocol.png";
import Daniola from "@/public/images/daniola.png";
import CartAll from "@/public/images/cartall.png";
          ...
          {recent_works.map((x, i) => (
            <div key={i}>
              <Link href={x.href} target="_blank">
                ...
                >
                  <Image src={x.image_src} alt={x.alt} className="rounded-lg" />
                </div>
              </Link>
              <h4 className="font-medium mt-2 text-[min(4vw,24px)]">
                {x.title}
              </h4>
              <h5 className="font-normal text-[min(3vw,16px)]">{x.summary}</h5>
            </div>
          ))}
        </div>
      </div>
