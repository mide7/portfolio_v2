...
import CircleFilledSvg from '../svgs/circleFill'
import CircleOutlinedSvg from '../svgs/circleOutline'
...
export default function Testimonials() {
    ...
    function handleComment(i: number, currentComment: number) {
        const clientContainerWidth = clientCommentContainerRef.current?.clientWidth || 0
        if (i < currentComment) {
    ...
    useEffect(() => {
        ...
        function callback(entries: any) {
            if (!initial) {
                entries.forEach((entry: { isIntersecting: any; target: { dataset: { id: string | number } } }) => {
    ...
    useEffect(() => {
        clientCommentContainerRef.current && clientCommentContainerRef.current.scrollBy(-9999, 0)
    }, [])
                ...
                <div className='mx-auto w-full lg:max-w-xl space-y-3 py-3 no-scroll-bar '>
                    ...
                    >
                        {TestimonialsData.map((x, i) => (
                            <SingleTestimonial key={i} fullName={x.fullName} image={x.image} jobTitle={x.jobTitle} title={x.title} index={i} />
                        ))}
                    </div>
                    <span className='flex justify-center gap-1'>
                        {TestimonialsData.map((_x, i) => (
                            <button key={i} type='button' onClick={() => handleComment(i, currentComment)} className='flex items-center transition-all duration-300'>
                                {
                                    i === currentComment ?
